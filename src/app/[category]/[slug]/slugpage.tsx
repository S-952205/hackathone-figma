...
async function getdetail(slug: string) {
  try {
    ...
    console.log("error fetching slug data from sanity", error);
    return[]; // Return an empty array if an error occurs
  }
  
}

// Fetch a list of products for related suggestions
async function getproducts() {
  try {
    const query =
      await client.fetch(`* [_type == 'products' && title in ['Casual Green Bomber Jacket',
    ...
    console.log("error fetching sepcific data from sanity", error);
    return[]; // Return an empty array if an error occurs
  }
...
const Slugpage = ({ params }: { params: { slug: string } }) => {
  ...
  //try-catch ka kaam hai errors handle karna. Jab aap code likhte hain jo fail ho sakta hai (jaise data fetch karna),
  // toh agar error aaye toh program crash na kare
  useEffect(() => {
 ...
 console.log('product id', products._id);
  ...
  const [specific, setSpecific] = useState<Product[]>([]); //empty array shuru main ([])
  ...
  useEffect(() => {
    /**products backend ya kisi API se aata hai, aur yeh asynchronous (time lagane wala) data ho sakta hai.
       Jab tak products ka naya data nahi milta, cartItem state ko update karna zaruri nahi hai.
       Scenario 1: Product ka data load ho raha hai (loading state). Is time par koi state update nahi karni
      chahiye, warna empty ya galat data UI me dikh jayega
      Maan lijiye ek product page open kiya. Us product ki details (price, title, etc.) backend se aa rahi hain.
      Jab tak details load nahi hoti, cartItem state khali rahega.
      Jab nayi details aati hain, useEffect state update karta hai, aur woh UI me dikhata hai */
    if (products && products._id) { //Yeh check karta hai ki products ka data load ho chuka hai ya nahi

      /*Yeh function state ko update karta hai. Jaise hi products ka data aata hai,
       cartItem object ko backend se aaye data ke sath update karte hain */
       //State Update: Sirf wahi details state me set hongi jo API call se mili hain means data products ka 
      setCartItem({
        id: products._id,
        title: products.title,
        image: products.imageUrl,
        price: products.price,  // Default to 0 if price is not available
        discount: products.discountpercent,  // Default to 0 if no discount percent
        slug: products.slug,
        category: products.category, // Default to empty if no category
        color: products.color[0],    // Default to empty string if color is undefined
        size: products.size[0],   // Default to empty if no size is selected
        quantity: 1,             // Default quantity
      });
    }
  }, [products]); // Dependency array ensures this runs when 'product' changes
  
  console.log('cartitem', products._id);
        ...
        <div className='w-[600px]  flex flex-col gap-[10px]'>
            ...
            <div className='flex flex-row items-center gap-[14px]'>
              {cartItem.discount > 0 &&
                <p className='font-Satoshi font-[700] text-[32px] text-black'>
                  ${(cartItem.price - (cartItem.price * cartItem.discount) / 100 ) * cartItem.quantity}</p>}

              <p className={`font-Satoshi font-[700] text-[32px] text-black
              ${cartItem.discount > 0 && 'line-through text-black/30 decoration-2'}`}>
                ${cartItem.price * cartItem.quantity}</p>
            </div>

            {
              cartItem.discount > 0 && <div className='h-[34px] px-[6px] py-[14px] rounded-[62px] bg-[#FF33331A]
            flex flex-row justify-center items-center'>
                <p className='text-[#FF3333] font-Satoshi font-[500] text-[16px]'>-{cartItem.discount}%</p>
              </div>
            }
          </div>
          {/**Description */}
              ...
              <div className='w-[143px] h-[37px] flex flex-row justify-between mt-2'>
                {products.color.map((item: string, i: number) => (
                  ...
                  onClick={() => setCartItem({ ...cartItem, color: item })}
                    ...
                    key={i} className="rounded-full w-[37px] h-[37px] focus:border-[2px] focus:ring-2
            ...
            <div className='w-[420px] h-[46px] flex flex-row justify-between mt-2'>
              {products.size && products.size.length > 0 ? (
                products.size.map((item: string, i: number) => (
                  <button onClick={() => setCartItem({ ...cartItem, size: item })}
                    key={i}                    
          ...
          <div className='flex flex-row justify-between mt-1'>
            <div className='w-[170px] h-[52px] py-[16px] px-[20px] rounded-[62px] bg-[#F0F0F0]
              ...
              <button
               onClick={() => setCartItem({...cartItem,quantity: cartItem.quantity <= 1 ? 1 : cartItem.quantity - 1 })}>
                <Minus color="black" size={18} />
              </button>
              <p className='font-Satoshi font-[500] text-[16px] text-black'>{cartItem.quantity}</p>
